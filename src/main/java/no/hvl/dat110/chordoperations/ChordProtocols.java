...
public class ChordProtocols {
	...
	public void fixFingerTable() {
		...
		try {
			...
			List<NodeInterface> fingerTable = this.chordnode.getFingerTable();
			fingerTable.clear();
			BigInteger addressSize = Hash.addressSize();
			int numberofBits = Hash.bitSize();

			for (int i = 0; i < numberofBits; i++) {
				BigInteger succ = chordnode.getNodeID().add(BigInteger.valueOf(2)).pow(i).mod(addressSize);
				NodeInterface successor = chordnode.findSuccessor(succ);
				if(successor != null) {
